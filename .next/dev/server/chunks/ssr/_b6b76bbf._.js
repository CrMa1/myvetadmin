...
function ConsultasPage() {
    ...
    const handleSubmit = async (e)=>{
        ...
        if (result.success) {
            ...
            alert(result.error || "Error al guardar consulta");
        }
    };
    const handleDelete = async (id)=>{
        if (confirm("¿Estás seguro de eliminar esta consulta?")) {
            const userId = getUserId();
            const clinicId = getClinicId();
            await fetch(`/api/consultations?id=${id}&userId=${userId}&clinicId=${clinicId}`, {
                method: "DELETE"
            });
            fetchConsultations();
        }
    ...
    const handleNewPatient = async (e)=>{
        ...
        if (!newPatientData.name || !newPatientData.animalType || !newPatientData.ownerName) {
            alert("Por favor completa los campos obligatorios del paciente");
            return;
        }
        const userId = getUserId();
        const clinicId = getClinicId();
...
//# sourceMappingURL=_b6b76bbf._.js.map